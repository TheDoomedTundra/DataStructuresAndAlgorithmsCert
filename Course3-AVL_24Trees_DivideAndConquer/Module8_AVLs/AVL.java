...
public class AVL<T extends Comparable<? super T>> {
    ...
    private AVLNode<T> addImpl(T data, AVLNode<T> curr) {
        if (curr == null) {
            size++;
            curr = new AVLNode<>(data);
        } else if (data.compareTo(curr.getData()) < 0) {
            curr.setLeft(addImpl(data, curr.getLeft()));
        } else if (data.compareTo(curr.getData()) > 0) {
            curr.setRight(addImpl(data, curr.getRight()));
        }
    ...
    public T remove(T data) {
        // WRITE YOUR CODE HERE (DO NOT MODIFY METHOD HEADER)!
        assertNotNull(data);
        AVLNode<T> tmp = new AVLNode<>(data);
        root = removeImpl(root, data, tmp);
        return tmp.getData();
    }

    private AVLNode<T> removeImpl(AVLNode<T> curr, T data, AVLNode<T> dummy) {
        if (curr == null) {
            ...
            if (curr.getLeft() == null && curr.getRight() == null) {
                ...
                AVLNode<T> dummy2 = new AVLNode<>(curr.getData());
    ...
    private void updateHeightAndBF(AVLNode<T> currentNode) {
        // WRITE YOUR CODE HERE (DO NOT MODIFY METHOD HEADER)!
        int heightRight = (currentNode.getRight() != null) ? currentNode.getRight().getHeight() : -1;
        int heightLeft = (currentNode.getLeft() != null) ? currentNode.getLeft().getHeight() : -1;

        currentNode.setHeight(Math.max(heightLeft, heightRight) + 1);
        currentNode.setBalanceFactor(heightLeft - heightRight);
    }
